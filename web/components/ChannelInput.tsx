...
export default function ChannelInput() {
  ...
  const onAdd = async () => {
    ...
    try {
      ...
      const metaRes = await fetch(
        `/api/youtube/channel?id=${encodeURIComponent(channelId)}`
      );
      ...
      const vidsRes = await fetch(
        `/api/youtube/videos?id=${encodeURIComponent(channelId)}`
      );
      if (vidsRes.ok) {
  ...
  return (
    <div className="flex w-full gap-2">
      ...
      >
        {loading ? (
          <span className="h-4 w-4 animate-spin rounded-full border-2 border-background/50 border-t-transparent inline-block" />
        ) : (
          "+"
        )}
        Add
