...
function formatDuration(seconds?: number) {
  ...
  const parts = [h > 0 ? String(h) : null, String(h > 0 ? m.toString().padStart(2, "0") : m), String(s).padStart(2, "0")].filter(Boolean);
  return parts.join(":");
}

export default function VideoCard({ channelId, videoId }: { channelId: string; videoId: string }) {
  const [open, setOpen] = useState(false);
  const video = useStore((s) => s.videosByChannel[channelId]?.find((v) => v.id === videoId));
    ...
    <div className="rounded border p-2">
      <div className="grid grid-cols-[160px_1fr] gap-3 items-center">
        <div className="relative w-[160px] h-[90px]">
          <Image src={video.thumbnail} alt={video.title} fill className="object-cover rounded" />
        </div>
        <div className="space-y-1">
          ...
          <div className="flex gap-2 text-xs">
            <button className="rounded bg-green-600 text-white px-2 py-1" onClick={() => onMark("watched")}>
              Watched
            </button>
            <button className="rounded bg-yellow-600 text-white px-2 py-1" onClick={() => onMark("skipped")}>
              Skip
            </button>
            <button className="rounded border px-2 py-1" onClick={() => setOpen((o) => !o)}>
              {open ? "Hide" : "Play"}
            </button>
          </div>
        </div>
